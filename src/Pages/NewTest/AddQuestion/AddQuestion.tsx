...
export const AddQuestion: FC<AddQuestionProps> = ({ testId }) => {
  ...
  const createNewQuestion = () => {
    ...
    QuestionService.createNewQuestion(testId, question)
      .then((res) => {
  ...
  const deleteQuestion = (id: number) => {
    QuestionService.deleteQuestion(testId, id)
      .then((res) => {
        getQuestions(testId);
        setQuestionId(null);
  ...
  const startEditQuestion = () => {
    ...
    const editQuestion = questionList.filter((elem) => elem.id === questionId)[0];
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className='questionWrap'>
        ...
        <Modal visible={isModalVisible} onOk={handleModal} onCancel={handleModal}>
